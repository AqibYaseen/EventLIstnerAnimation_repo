

let innerDiv = document.getElementById('innerDiv');
let innerDiv2 = document.getElementById('innerDiv2');


innerDiv.onmouseover = () => {
    innerDiv.style.width = '600px';
    innerDiv.style.backgroundColor = 'grey';
    innerDiv.style.borderStyle = 'solid';
    innerDiv.style.borderRadius = '100%';    
} 
innerDiv.onmouseout = () => {
    innerDiv.style.borderRadius = '0%';    
 }
innerDiv.onclick = () => {
    innerDiv.style.width = '700px';
    innerDiv.style.backgroundColor = 'grey';
    innerDiv.style.borderStyle = 'solid';
    innerDiv.style.borderRadius = '0%';    
}
innerDiv.ondblclick = () => {
    innerDiv.style.width = "600px"
}

innerDiv2.onmouseover = () => {
    innerDiv2.style.borderRadius = "50%";
}
innerDiv2.onmouseout = () => {
    
    innerDiv2.style.borderRadius = "0%";
    innerDiv2.style.borderStyle = "solid";
    innerDiv2.style.borderColor = "black"
}


















// function currentTime() {
// //     let date = new Date(); 
// //     let hh = date.getHours();
// //     let mm = date.getMinutes();
// //     let ss = date.getSeconds();
// //     let session = "AM";
  
// //     if(hh === 0){
// //         hh = 12;
// //     }
// //     if(hh > 12){
// //         hh = hh - 12;
// //         session = "PM";
// //      }
  
// //      hh = (hh < 10) ? "0" + hh : hh;
// //      mm = (mm < 10) ? "0" + mm : mm;
// //      ss = (ss < 10) ? "0" + ss : ss;
      
// //      let time = hh + ":" + mm + ":" + ss + " " + session;
  
// //     document.getElementById("innerDiv2").innerText = time; 
// //     let t = setTimeout(function(){ currentTime() }, 1000);
// //   }
  
// //   currentTime();
  